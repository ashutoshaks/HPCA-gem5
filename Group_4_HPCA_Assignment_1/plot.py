...
parser.add_argument(
    "-d", help="Folder containing stats for different configurations", required=True)
dir_path = vars(parser.parse_args())['d']
gem5_dir = os.path.expanduser(vars(parser.parse_args())['g'])
PLOT_DIR = os.path.join(gem5_dir, 'configs/assignment/plots')
...
if __name__ == '__main__':
    ...
    for key in tqdm(OUTPUT_FIELDS):
        ...
        for index, data in enumerate(y_data):
            plt.text(index + .5, data + 0.01 *
                     data_variations, '{0:10}'.format(data))
        plt.xlabel("configs")
        plt.ylabel(key)
        plt.xticks(x_data)
    ...
    print(
        f"You can find the plots in the following directory: {PLOT_DIR}", file=sys.stderr)
